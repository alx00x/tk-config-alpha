...
class HieroUpdateCuts(HookBaseClass):
    ...
    def allow_cut_updates(self, preset_properties):
        ...
        """
        # CRATER addition ==============================================
        # The custom_create_cut_bool_property is a bool property that controls
        # whether we update cuts or not.
        return preset_properties.get("custom_create_cut_bool_property", True)
        # ==============================================================
    ...
    def create_cut_item(self, cut_item_data, preset_properties):
        ...
        """
        # CRATER addition ==========================================================================================
        # if transcode is not checked in export options cut_item gets linked to the latest "edit" version  
        if not preset_properties.get("custom_transcode_bool_property", True):
            engine = sgtk.platform.current_engine()
            sg = engine.shotgun

            additional_filter_presets = [ {"preset_name": "LATEST", "latest_by":   "ENTITIES_CREATED_AT"} ]
            filters = [   ['project', 'is', cut_item_data['project']], 
                          ['entity', 'is', cut_item_data['shot']], 
                          ['code', 'ends_with', 'edit']  ] 
            fields = ['code']
            versions = sg.find('Version', filters = filters, additional_filter_presets = additional_filter_presets, fields=fields)
            cut_item_data['version'] = versions[0]
        # ===========================================================================================================    
